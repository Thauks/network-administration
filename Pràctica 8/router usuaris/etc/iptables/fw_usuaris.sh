...
iptables -A FORWARD -m conntrack --ctstate NEW -i eth-clients -o eth-dmz -j ACCEPT
iptables -A FORWARD -i eth-clients -o eth-clients -j ACCEPT

#permís als routers i al monitor de la dmz
iptables -A FORWARD -i eth-dmz -o eth-clients -s 10.10.2.1 -j ACCEPT
iptables -A FORWARD -i eth-dmz -o eth-clients -s 10.10.2.3 -j ACCEPT
iptables -A FORWARD -i eth-dmz -o eth-clients -s 10.10.2.11 -j ACCEPT
